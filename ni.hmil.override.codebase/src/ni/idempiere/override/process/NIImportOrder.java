 ...
 *****************************************************************************/
package ni.idempiere.override.process;
 ...
 */
public class NIImportOrder extends SvrProcess
{
...
/*		red1 20th December 2012 - Checking AD_Org for 0 or * is redundant as its set to that! Thus AD_Org_ID has to be reset from POS Locator (see lower)
...
*/
		 ...
		 * red1 - 20th December 2012 - Set also AD_Org_ID from its match. Need further review of other incidents.
		 */
		sql = new StringBuilder ("UPDATE I_Order o ")	//	 
...
/*		sql = new StringBuilder ("UPDATE I_Order o ")
			 .append("SET CountryCode=(SELECT MAX(CountryCode) FROM C_Country c WHERE c.IsDefault='Y'")
			.append(" AND c.AD_Client_ID IN (0, o.AD_Client_ID)) ")
			.append("WHERE C_BPartner_ID IS NULL AND CountryCode IS NULL AND C_Country_ID IS NULL")
			.append(" AND I_IsImported<>'Y'").append (clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		...
		try
		{
			...
			while (rs.next ())
			{
				...
				Object value=imp.get_Value("discount");						
				
				line.setPrice(imp.getPriceActual().subtract(value==null?BigDecimal.ZERO:new BigDecimal(""+value)));
				line.setPriceList(imp.getPriceActual()); 
				//if(line.getPriceActual().compareTo(BigDecimal.ZERO)!=0)
				//	line.setDiscount(value!=null?new BigDecimal(""+value).multiply(new BigDecimal(100)).divide(line.getPriceActual(),12,BigDecimal.ROUND_HALF_UP):BigDecimal.ZERO);
				...
				if (imp.save())
					noInsertLine++;
				count++;
				statusUpdate("Procesando registro # "+count+"(Orden de Venta="+order+" , linea "+lineNo+")");
