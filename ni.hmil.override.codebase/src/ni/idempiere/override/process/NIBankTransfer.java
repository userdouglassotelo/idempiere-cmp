...
public class NIBankTransfer extends SvrProcess
{
	...
	protected void prepare()
	{
			...
			
		{
			...
			if (name.equals("From_C_BankAccount_ID"))
				...
				C_Campaign_ID = para[i].getParameterAsInt();
			else
	...
	{ 
		...
		if(C_Campaign_ID!=0)
			paymentBankFrom.setC_Campaign_ID(C_Campaign_ID);
		...
		if(C_Campaign_ID!=0)
			paymentBankTo.setC_Campaign_ID(C_Campaign_ID);
	...
	/*
	public void setC_DocType_ID(MPayment payment, boolean isReceipt ) 
	{
		payment.setIsReceipt(isReceipt);
		String sql = "SELECT C_DocType_ID,istransfer FROM C_DocType WHERE IsActive='Y' AND AD_Client_ID=? AND DocBaseType=? ORDER BY IsDefault DESC";
		...
		try
		{
			...
			while (rs.next())
			{ 
				if("Y".equals(rs.getString("istransfer")))
					payment.setC_DocType_ID(rs.getInt(1));
			}
